...
//定义设备的名字为 pwm
#define DEVICE_NAME				"s3c24xx-pwm"
...
//蜂鸣器GPIO配置
#define BUZZER_PMW_GPIO			EXYNOS4_GPD0(2)
...
//设置频率
static void pwm_set_freq(unsigned long duty) {
//PWM的占空比的配置
	int period_ns = NS_IN_1HZ / 50;
	
	pwm_config(pwm4buzzer, period_ns / 200*duty, period_ns); 
...
static  void pwm_stop(void) {
	s3c_gpio_cfgpin(BUZZER_PMW_GPIO, S3C_GPIO_OUTPUT);
		...
		pwm_config(pwm4buzzer, 0, NS_IN_1HZ / 50);
